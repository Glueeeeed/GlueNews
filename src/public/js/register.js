...
async function decryptAesGcm(cipherTextBase64, keyHex) {
    const key = Uint8Array.fromHex(keyHex);
    const [cipherTextPart, nonceHex] = cipherTextBase64.split(":");
    const nonce = Uint8Array.fromHex(nonceHex);
    const cipherText = Uint8Array.fromBase64(cipherTextPart);
    const aes = gcm(key, nonce);
